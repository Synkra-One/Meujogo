...
function Targeting.Hits(origin: Vector3, direction: Vector3, target: Model, params: RaycastParams): boolean
	...
	for _, name in { "Head", "UpperTorso", "Torso", "HumanoidRootPart" } do
		...
		if part and part:IsA("BasePart") and Rules.InCone(origin, direction, part.Position) then
			local result = Workspace:Raycast(origin, part.Position - origin, params)
			if result and result.Instance:IsDescendantOf(target) then return true end
		end
	end
	return false
end
